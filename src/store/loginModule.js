...
const actions = {
  login ({ commit }, user) {
    return new Promise((resolve, reject) => {
      commit('auth_request')
      axios({
        url: 'http://barber.brozapp.com/api/login',
        data: user,
        method: 'POST'
      })
        .then(response => {
        ...
        //   const type = JSON.stringify(response.data.userInfo.type)
          ...
          //   localStorage.setItem('timeSlotCache', JSON.stringify(response.data.timeList))
          ...
          //   store.dispatch('setTimeSlot', JSON.stringify(response.data.timeList))
  ...
  vendor ({ commit }, requestData) {
    return new Promise((resolve, reject) => {
      commit('auth_request')
      axios({
        url: 'http://barber.brozapp.com/api/vendor/' + requestData,
        method: 'GET'
      })
        .then(response => {
